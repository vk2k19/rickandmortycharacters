...
const Card = (props) => {
  return (
    <div
      className={`card bg-dark bg-gradient rounded-2 border-secondary overflow-hidden h-100 ${props.className}`}
      data-testid="card"
    >
      <figure className="position-relative m-0" data-testid="card-img-title">
        ...
        <figcaption
          className="position-absolute bottom-0 pt-3 rounded-top-5 text-center bg-black-opacity text-white w-100"
          data-testid="card-title-wrapper"
        >
          <h2 className="m-0 h4" data-testid="card-title">
            {props.name}
          </h2>
          <span data-testid="card-id">id: {props.id}</span> -{" "}
          <span className="card-date">{getCreationText(props.created)}</span>
        </figcaption>
      </figure>
      <DetailTable {...props} />
    </div>
