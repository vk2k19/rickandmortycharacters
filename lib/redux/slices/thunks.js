...
const getAllCharacters = async (totalCharacters = 0) => {
  if (totalCharacters > 0) {
    const characterList = [...Array(totalCharacters)]
      .map((_, i) => i + 1)
      .join(",");
    const allCharacter = await fetch(
      `https://rickandmortyapi.com/api/character/${characterList}`
    );
...
export const getSortOptionsAsync = createAppAsyncThunk(
  "get/sort-options",
  async () => {
...
export const getCharactersAsync = createAppAsyncThunk(
  "get/characters",
  getCharacters
);
